...
import { FiArrowLeft, FiSearch, FiTrash2, FiArchive, FiMail, FiRefreshCw } from "react-icons/fi";
...
const Inbox = () => {
  ...
  const renderConversation = (email) => (
    <div key={email._id} className="mb-6 bg-white border border-gray-200 rounded shadow-sm">
      ...
      <div 
        ...
        dangerouslySetInnerHTML={{ __html: sanitizeHtml(email.htmlBody || email.textBody || "") }}
      />
    ...
    <div className="flex bg-[#f3f2f1] h-screen overflow-hidden font-sans text-gray-900">
      ...
      <main className="flex-1 flex md:ml-64 flex-col min-w-0">
        ...
        <header className="h-12 bg-white border-b flex items-center px-4 gap-6 text-sm text-gray-700 z-10">
          ...
          <button onClick={fetchEmails} className="flex items-center gap-2 hover:bg-gray-100 p-1 px-2 rounded">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* List Pane */}
          <section className={`${isMobileView && selectedEmail ? "hidden" : "flex"} flex-col w-full md:w-[350px] lg:w-[400px] border-r bg-white`}>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className={`text-sm truncate ${!isRead ? "font-bold text-blue-700" : "font-semibold text-gray-900"}`}>
                    ...
                    <div className={`text-sm truncate ${!isRead ? "font-semibold" : "text-gray-700"}`}>
          ...
          {/* Reading Pane */}
          <section className={`${isMobileView && !selectedEmail ? "hidden" : "flex"} flex-1 flex-col bg-white overflow-hidden`}>
            {selectedEmail ? (
              <div className="flex-1 flex flex-col overflow-hidden">
                {/* Email Header */}
                <div className="p-6 border-b shrink-0">
                  {isMobileView && (
                    <button onClick={() => setSelectedEmail(null)} className="flex items-center gap-2 text-blue-600 mb-4 text-sm">
                      <FiArrowLeft /> Back to Inbox
                    </button>
                  )}
                  <h1 className="text-xl font-light text-gray-900 mb-4">{selectedEmail.subject || "No Subject"}</h1>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-sm">{selectedEmail.senderAddress}</div>
                      <div className="text-xs text-gray-500">To: {selectedEmail.recipientAddress || "me"}</div>
                    </div>
                  </div>
                </div>
