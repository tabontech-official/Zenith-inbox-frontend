...
const closeModal = () => {
  ...
  const getThreadMessages = (email) => {
    ...
    const childMessages = (email.children || []).map((child) => ({
      ...child,
      isParentMessage: false,
    }));

    return [parentMessage, ...childMessages].sort(
      (a, b) =>
...
showModal({
  ...
  const filteredEmails = emails.filter((email) => {
    ...
    return (
      email.senderAddress?.toLowerCase().includes(term) ||
      email.recipientAddress?.toLowerCase().includes(term) ||
      email.subject?.toLowerCase().includes(term) ||
      email.textBody?.toLowerCase().includes(term) ||
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-1.5">
                              ...
                              {email.children?.length > 0 && (
                                <span className="inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700">
                                  {email.children.length} Reply
                                  {email.children.length > 1 ? "ies" : ""}
                                </span>
                              )}
