...
const SetupFlow = () => {
  ...
  useEffect(() => {
    if (step === 3 && user?._id) {
      ...
      const fetchVerification = async () => {
        try {
          ...
          if (data.success && data.data) {
            ...
            // ✅ if email found, stop checking and update state
            setVerificationEmail({
              ...email,
              toEmail: autoEmail || email.toEmail || "",
              isGmailVerification,
            ...
            if (attempts >= maxAttempts) {
              clearInterval(intervalId);
              // ❌ no email after 5 attempts — show input + enable validate button
              setVerificationEmail({
                toEmail: "",
                formattedBody: "",
      ...
      // run every 10s until success or 5 attempts done
      const intervalId = setInterval(fetchVerification, 10000);
      fetchVerification(); // run immediately first time
    ...
    const fetchVerification = async () => {
      ...
      try {
        ...
        if (data.success && data.data) {
          ...
          // ✅ Verification email found — stop checking
          setVerificationEmail({
            ...email,
            toEmail: autoEmail || email.toEmail || "",
            isGmailVerification,
        ...
        // ❌ No mail found — after maxAttempts, stop loop
        if (attempts >= maxAttempts) {
          clearInterval(intervalId);
          setShowValidateButton(true); // show input & enable validate button
          setVerificationEmail({
            toEmail: "",
            formattedBody: "",
            isGmailVerification: false,
  ...
  const startValidationLoop = async () => {
    ...
    const maxAttempts = 5;
    let stopped = false;
    let timeoutId = null; // 👈 keep reference to timeout
    ...
    const checkValidation = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          stopped = true;
          clearTimeout(timeoutId); // 🛑 stop pending loop
          setValidated(true);
          setValidating(false);
          setValidationPhase(false);
        ...
        if (attempts < maxAttempts && !stopped) {
          ...
          stopped = true;
          clearTimeout(timeoutId); // 🛑 stop any pending loop
          setValidating(false);
          setValidationPhase(false); // hide loader
          setValidationFailed(true); // show red box
          toast.error(
