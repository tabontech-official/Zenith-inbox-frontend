  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col">
        ...
        {/* --- Canva / FlowNode Style Layout --- */}
{/* --- Canva-Style Node Layout --- */}
      ...
      {routerBranches.map((branch, branchIndex) => (
        <div key={branch.id} className="flex flex-col items-center space-y-8">
              ...
              return (
                <React.Fragment key={module.id}>
                  <FlowNode
                    ...
                    onDelete={() => {
                      const updated = [...routerBranches];
                      updated[branchIndex].modules =
                        updated[branchIndex].modules.filter(
                          (_, i) => i !== moduleIndex
                        );
                      setRouterBranches(updated);
                    }}
          ...
          <button
            onClick={() => {
              setEditingBranch(branchIndex);
              setOpen(true); // 👈 this opens your existing Add Module modal
            }}
            className="w-64 h-16 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center hover:border-purple-500 hover:bg-purple-50 transition-all"
          >
            <Plus className="w-5 h-5 text-gray-400 mr-2" />
            <span className="text-gray-500 font-medium">Add Module</span>
          </button>
        </div>
      ))}

      {/* Add Branch Button (if you want new branches under Router) */}
      <div className="flex justify-center mt-10">
        <button
          onClick={addRouterBranch}
          className="px-4 py-2 bg-green-100 border-2 border-green-400 text-green-700 rounded-lg hover:bg-green-200 transition-all flex items-center"
        >
