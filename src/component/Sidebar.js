...
const Sidebar = () => {
  ...
  const SidebarHeader = ({ title, Icon }) => (
    <div className="flex items-center justify-between py-5 px-6 bg-white">
      <div className="flex items-center space-x-2">
        <Icon className="text-indigo-500 text-2xl" />
        <span className="font-semibold text-lg text-gray-800">{title}</span>
      </div>
      ...
      <button
        onClick={() => setIsSidebarOpen(false)}
        className="md:hidden text-gray-600 hover:text-black p-1 rounded-full transition-colors"
        aria-label="Close sidebar"
      >
        <FiX className="w-6 h-6" />
      </button>
  ...
  return (
    <>
      <button
        onClick={() => setIsSidebarOpen(true)}
        className={`fixed top-4 right-4 z-50 p-2 rounded-full bg-white text-black md:hidden transition-opacity duration-300 ${
          isSidebarOpen ? "opacity-0 pointer-events-none" : "opacity-100"
        }`}
      >
        <FiMenu className="w-6 h-6" />
      ...
      <div
        className={`fixed inset-0 z-30 bg-black bg-opacity-30 md:hidden transition-opacity duration-300 ${
          isSidebarOpen ? "opacity-100 visible" : "opacity-0 invisible"
        }`}
        onClick={() => setIsSidebarOpen(false)}
      />
      ...
      <aside
        className={`fixed z-40 top-0 left-0 h-full w-64 bg-white text-gray-800 flex flex-col transition-transform duration-300 md:translate-x-0 ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {role === "admin" ? renderAdminSidebar() : renderUserSidebar()}
        ...
        <div className="px-4 pb-4">
          <button
            onClick={handleLogout}
            className="flex items-center w-full space-x-3 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-gray-100 hover:text-black transition-colors duration-200"
          >
            <FiLogOut className="w-5 h-5" />
            <span>Logout</span>
          </button>
        ...
        <div className="text-xs text-gray-400 py-3 text-center bg-white">
          © 2025 MailMatrix
        </div>
      </aside>
    </>
